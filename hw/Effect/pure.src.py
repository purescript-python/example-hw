...
res = block( " | This module provides the `Effect` type, which is used to represent | _native_ effects. The `Effect` type provides a typed API for effectful | computations, while at the same time generating efficient JavaScript."
           , assign_star( "$foreign"
                        , call(var('import_module'), "hw.ffi.Effect") )
           , assign_star( "ps_Control_Applicative"
                        , call( var('import_module')
                              , "hw.Control.Applicative.pure" ) )
           , assign_star( "ps_Control_Apply"
                        , call(var('import_module'), "hw.Control.Apply.pure") )
           , assign_star( "ps_Control_Bind"
                        , call(var('import_module'), "hw.Control.Bind.pure") )
           , assign_star( "ps_Control_Monad"
                        , call(var('import_module'), "hw.Control.Monad.pure") )
           , assign_star( "ps_Data_Functor"
                        , call(var('import_module'), "hw.Data.Functor.pure") )
           , assign_star( "ps_Data_Monoid"
                        , call(var('import_module'), "hw.Data.Monoid.pure") )
...
res = module_code(res, filename="C:\\Users\\twshe\\Desktop\\mydb\\com-haskell\\v0.1\\hw\\.spago\\effect\\v2.0.1\\src\\Effect.purs", name="hw.Effect.pure")
